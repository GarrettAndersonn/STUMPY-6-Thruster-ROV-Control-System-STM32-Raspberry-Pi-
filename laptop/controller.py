...
def main():
    print("RUNNING STUMPY CONTROLLER")
    ...
    assist_mode = False
    arm_time = 0.0
    last_button_time = {"arm": 0.0, "disarm": 0.0, "stop": 0.0, "mode": 0.0}
    ...
    while True:
        ...
        if js.get_button(BUTTON_MODE_TOGGLE) and (now - last_button_time["mode"] > BUTTON_COOLDOWN):
            assist_mode = not assist_mode
            if assist_mode:
                send_line(sock, "MODE:ASSIST")
                print("Sent: MODE:ASSIST")
            else:
                send_line(sock, "MODE:MANUAL")
                print("Sent: MODE:MANUAL")
            last_button_time["mode"] = now
        ...
        mode_name = "ASSIST" if assist_mode else "MANUAL"
